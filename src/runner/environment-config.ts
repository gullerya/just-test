...
function setExecutionContext(mode, testId: any = null, startHandler?, endHandler?, key: symbol = EXECUTION_CONTEXT_SYMBOL) {
	let result;
	switch (mode) {
	...
	(globalThis as any)[key] = result;
	return result;
}

function getExecutionContext(key: symbol = EXECUTION_CONTEXT_SYMBOL) {
	return (globalThis as any)[key] ?? { mode: EXECUTION_MODES.PLAIN_RUN };
}
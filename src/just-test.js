...
const
	...
	DEFAULT_SYNC_TEST_TTL = 10 * 1000,
	DEFAULT_ASYNC_TEST_TTL = 30 * 60 * 1000;
...
function Test(options, testCode) {
	let id, name, async, skip, ttl, status = 'idle', message, startTime, beg, end, duration, view, tmp;
	...
	ttl = typeof options.ttl === 'number' ? options.ttl : (async ? DEFAULT_ASYNC_TEST_TTL : DEFAULT_SYNC_TEST_TTL);
	...
	tmp.onclick = function () {
		let cntnt, msgEl = view.querySelector('.message');
	...
	function run() {
		...
		if (skip) {
			...
			testPromise = new Promise((resolve, reject) => {
				...
				testCode(resolve, reject);
			});
			testPromise.then(
				msg => { finalize('passed', msg); },
				msg => {
					let error = msg instanceof Error ? msg : new Error(msg);
...
function Suite(options) {
	let id, name, tests = [], passed = 0, failed = 0, skipped = 0, status = 'idle', startTime, beg, end, duration,
		view, tmp;
	options = typeof options === 'object' ? options : {};
	if ('id' in options) id = options.id;
	name = 'name' in options ? options.name : DEFAULT_SUITE_NAME;
	...
	function addTest(options, testCode) {
		let em = 'bad parameters: must be 1 or 2 where the last one is a function', test;
		...
		suitePromise = new Promise(resolve => {
			...
			(function iterate(index) {
				...
				if (index === tests.length) {
					...
					if (test.async) {
						asyncFlow = asyncFlow.then(() => { return testPromise; });
						iterate(index);
					}
				}
			})(0);
...
function buildView() {
	let css, jtUI, jtList, tmp, startX, startY, startLeft, startTop, tmpMMH, state = 'max';

	function toggleMinMax(newState) {
		let b = jtUI.getElementsByClassName('minMaxToggle')[0];
	...
	tmp.onmousedown = function (event) {
		...
		document.onmousemove = function (event) {
			let top = startTop + event.clientY - startY, left = startLeft + event.clientX - startX;
			top = top < 0 ? 0 : top;
			left = left < 0 ? 0 : left;

...
- [ ] **3.4 No integration test for per-mode coverage semantics.** Add `tests/server/coverage-modes-test.ts` that asserts iframe → 1 `TN:__session__`, page → ≥1 per-test, worker → 0. Prevents silent regression. **Deferred** — to be done as part of a wider unit-test coverage effort.
---
(function () {
	...
	suite = new window.Utils.JustTest.Suite({
		name: 'validate Suite object'
	});

	suite.addTest({ description: 'JustTest namespace created okay' }, function (pass, fail) {
	...
	suite.addTest({ description: 'Suite object created (no options)' }, function (pass, fail) {
		...
		if (tmpSuite.name !== '') fail(new Error('name expected to be equal empty string when not explicitly stated'));
	...
	suite.addTest({ description: 'Suite object created (with options)' }, function (pass, fail) {
	...
	suite.addTest({ description: 'Suite object created (properties should be immutable)' }, function (pass, fail) {
